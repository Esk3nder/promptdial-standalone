...
export class Validator {
  ...
  async validate(response: unknown): Promise<StrategyPlannerResponse> {
    ...
    try {
      ...
      if (elapsedMs > this.maxValidationTimeMs) {
        console.warn(`Validation took ${elapsedMs}ms, exceeding ${this.maxValidationTimeMs}ms limit`);
      }
  ...
  private validateTechniquesAgainstAllowList(techniques: Technique[]): void {
    const invalidTechniques = techniques.filter(
      tech => !TECHNIQUE_ALLOW_LIST[tech]
    );

    if (invalidTechniques.length > 0) {
      throw new ValidationError(
        `Techniques not in allow-list: ${invalidTechniques.join(', ')}`,
        { invalidTechniques }
      );
    }
  }
  ...
  private validateBusinessRules(response: StrategyPlannerResponse): void {
    ...
    for (let i = 0; i < suggested_techniques.length; i++) {
      ...
      if (metadata.incompatibleWith) {
        const conflicts = suggested_techniques.filter(
          t => metadata.incompatibleWith?.includes(t)
        );
  ...
  quickValidate(techniques: string[]): boolean {
    try {
      ...
      return techniques.every(
        tech => typeof tech === 'string' && tech in TECHNIQUE_ALLOW_LIST
      );
    } catch {
      return false;
    }
