...
export class LLMRunner {
  ...
  async runSingle(
    ...
    try {
      ...
      response.cost_usd = provider['calculateCost'](response.tokens_used, response.model)
   ...
   */
  async runBatch(variants: PromptVariant[], traceId?: string): Promise<LLMResponse[]> {
    logger.info(`Running batch of ${variants.length} variants`)

    const promises = variants.map((variant) =>
      this.runSingle(variant, false, undefined, traceId).catch((error) => {
  ...
  private async logEvent(
    ...
    try {
      const event = createTelemetryEvent('variant_generated', traceId, variantId, {
...
export async function handleRunBatchRequest(
  ...
  try {
    ...
    const serviceError = createServiceError(ERROR_CODES.INTERNAL_ERROR, 'Failed to run batch', true)
    return createServiceResponse(request, undefined, serviceError)
  }
}
...
export async function handleSelfConsistencyRequest(
  ...
  try {
    ...
    const result = await getLLMRunner().runSelfConsistency(variant, num_samples, trace_id)
