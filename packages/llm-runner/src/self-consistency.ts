...
import { PromptVariant, createLogger } from '@promptdial/shared'

// Define the self-consistency vote prompt locally
const SELF_CONSISTENCY_VOTE_PROMPT = `You are given multiple solutions to the same problem. Your task is to:
...
Output only the selected answer without any additional explanation.`
...
export class SelfConsistencyHandler {
   ...
   */
  async execute(variant: PromptVariant, numSamples: number = 3): Promise<SelfConsistencyResult> {
