...
export class SafetyGuard {
  ...
  async checkSafety(
    ...
    const shouldBlock = this.sanitizer.shouldBlock(result) || 
                       result.risk_score >= this.blockThreshold
  ...
  getSecurityStats(timeRange?: { start: Date; end: Date }): SecurityStats {
    ...
    const modifiedCount = entries.filter(e => 
      e.sanitized_prompt !== e.original_prompt
    ).length
    ...
    const avgRiskScore = entries.length > 0
      ? entries.reduce((sum, e) => sum + e.risk_score, 0) / entries.length
      : 0
