...
async function demonstrateOptimizer() {
  ...
  defaultResult.pareto_frontier.forEach(sol => {
    console.log(`  - ${sol.variant_id}: Quality=${sol.objectives.quality.toFixed(2)}, Cost=${sol.objectives.cost.toFixed(3)}, Latency=${sol.objectives.latency.toFixed(3)}`)
  })
  ...
  console.log(`Recommended: ${budgetResult.recommended.variant_id} (Cost: $${budgetResult.recommended.variant?.cost_usd})`)
  ...
  console.log(`Recommended: ${speedResult.recommended.variant_id} (${speedResult.recommended.variant?.latency_ms}ms)`)
