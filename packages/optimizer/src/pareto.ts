...
export class ParetoOptimizer {
  ...
  findParetoFrontier(solutions: ParetoSolution[]): ParetoSolution[] {
    ...
    logger.info(`Found Pareto frontier with ${frontier.length} solutions out of ${solutions.length}`)
  ...
  selectByPreference(
    ...
    logger.info(`Selected variant ${bestSolution.variant_id} with utility ${bestUtility.toFixed(3)}`)
  ...
  applyConstraints(
    ...
    return solutions.filter(solution => {
      ...
      if (constraints.max_cost !== undefined &&
          solution.objectives.cost > constraints.max_cost) {
        return false
      }
