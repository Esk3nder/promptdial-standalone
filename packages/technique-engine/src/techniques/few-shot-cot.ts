...
6. Less direct sunlight = cooler temperatures = winter`,
  ...
  async generate(
    ...
    if (budget.remaining_cost_usd > variant1.cost_usd * 1.5) {
      const formattedPrompt = this.createFormattedFewShotCoT(base_prompt, taskExamples)
  ...
  private createBasicFewShotCoT(basePrompt: string, examples: Example[]): string {
...
3. Final answer${complexityHint}`
  ...
  private createFormattedFewShotCoT(basePrompt: string, examples: Example[]): string {
    const exampleSection = examples
