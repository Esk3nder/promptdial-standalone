...
import { TechniqueEngine, handleGenerateVariantsRequest, handleRegisterTechniqueRequest } from '../src/index'
import {
...
describe('TechniqueEngine', () => {
  ...
  describe('registerTechnique', () => {
    ...
    it('should throw error if technique name already exists', () => {
      ...
      expect(() => engine.registerTechnique(technique2))
        .toThrow('Technique duplicate is already registered')
    })
    ...
    it('should throw error if technique name conflicts with built-in', () => {
      ...
      expect(() => engine.registerTechnique(technique))
        .toThrow('Technique few_shot_cot is already registered')
    })
  })
    ...
    it('should filter out invalid variants', async () => {
      ...
      const MockInvalidTechnique = vi.fn().mockImplementation(() => ({
        ...
        generate: vi.fn().mockResolvedValue([
          { id: '', prompt: 'test', technique: 'invalid', est_tokens: 100, temperature: 0.5, cost_usd: 0.01 },
...
describe('Service API', () => {
  ...
  describe('handleRegisterTechniqueRequest', () => {
    ...
    it('should handle duplicate registration error', async () => {
      ...
      const response = await handleRegisterTechniqueRequest(
        createTestServiceRequest(technique)
      )
