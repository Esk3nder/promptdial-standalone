...
describe('API Server', () => {
  ...
  describe('GET /api/test/:testId', () => {
    it('should return 404 for non-existent test', async () => {
      const response = await request(app).get('/api/test/non-existent-test').expect(404)
  ...
  describe('startServer', () => {
    ...
    it('should use PORT environment variable if set', async () => {
      ...
      vi.doMock('../../src/api/server', async () => {
        ...
        function startTestServer() {
          testApp.listen(process.env.PORT || 3001, () => {
            console.log(
              `🚀 PromptDial API server running on http://localhost:${process.env.PORT || 3001}`,
            )
          })
        }
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('http://localhost:4000'))
