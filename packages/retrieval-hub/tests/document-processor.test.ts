  ...
  describe('processDocument', () => {
    ...
    it('should respect chunk overlap', async () => {
      ...
      for (let i = 1; i < chunks.length; i++) {
        ...
        expect(chunks[i - 1].content).toContain(chunks[i].content.split(' ')[0])
      }
    })
...
This is section 2 content.`
    ...
    it('should use provided document ID', async () => {
      const chunks = await processor.processDocument('Test content', { id: 'custom-doc-id' })
  ...
  describe('text cleaning', () => {
    ...
    it('should remove control characters', () => {
      const cleaned = (processor as any).cleanText('Text with\x00control\x01characters\x1F')
