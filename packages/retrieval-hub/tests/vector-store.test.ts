...
describe('InMemoryVectorStore', () => {
  ...
  describe('getStats', () => {
    it('should return store statistics', async () => {
      const docs = [
        createTestDocument({ id: 'doc1' }),
        createTestDocument({ id: 'doc2' })
      ]
      await store.indexBatch(docs)
  ...
  describe('similarity calculation', () => {
    it('should calculate cosine similarity correctly', () => {
      const similarity = (store as any).cosineSimilarity(
        [1, 0, 0],
        [1, 0, 0]
      )
      expect(similarity).toBe(1) // Identical vectors
      
      const similarity2 = (store as any).cosineSimilarity(
        [1, 0, 0],
        [0, 1, 0]
      )
      expect(similarity2).toBe(0) // Orthogonal vectors
      
      const similarity3 = (store as any).cosineSimilarity(
        [1, 0, 0],
        [-1, 0, 0]
      )
