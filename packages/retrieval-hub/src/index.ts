...
export class RetrievalHub {
  ...
  async indexDocuments(
    ...
    try {
      ...
      const chunks = await this.documentProcessor.processBatch(
        documents,
        {
      ...
      logger.info(`Indexed ${documents.length} documents as ${chunks.length} chunks in ${duration}ms`)
  ...
  private async postProcessResults(
    ...
    if (query.filters?.rerank) {
      result.documents = await this.rerankResults(
        result.documents,
        query.query
      )
    }
  ...
  private async rerankResults(
    ...
    return documents.map(doc => {
      const exactMatchBoost = doc.content.toLowerCase()
        .includes(query.toLowerCase()) ? 0.1 : 0
