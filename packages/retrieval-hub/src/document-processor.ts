...
export class DocumentProcessor {
  ...
  async processDocument(
    ...
    const processedContent = opts.cleanText 
    ...
    // Chunk the content
    const chunks = sections.length > 1
  ...
  private chunkBySections(
    ...
    const chunks: Array<{ text: string; metadata: any; start: number; end: number; section?: string }> = []
  ...
  private detectLanguage(text: string): string {
    ...
    const maxLang = Object.entries(scores)
      .sort(([, a], [, b]) => b - a)[0]
