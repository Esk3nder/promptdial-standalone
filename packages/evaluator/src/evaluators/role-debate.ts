...
export class RoleDebateEvaluator extends BaseEvaluator {
  ...
  async evaluate(
    ...
    const critiques = await this.gatherCritiques(
      variant.prompt,
      response,
      roles,
      taskMeta
...
- [List specific suggestions]`
    ...
    try {
      ...
      const result = await axios.post(`${llmRunnerUrl}/run`, {
  ...
  private async conductDebate(
    ...
    for (let i = 0; i < roles.length; i++) {
      ...
      const rebuttal = await this.getRebuttal(
        prompt,
        response,
        defender,
        otherCritiques
...
REASONING: [Brief explanation of the consensus score]`
    ...
    try {
      ...
      const score = scoreMatch 
        ? this.normalizeScore(parseInt(scoreMatch[1]), 1, 10)
        : 0.7
