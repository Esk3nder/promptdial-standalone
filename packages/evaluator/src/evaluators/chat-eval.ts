...
export class ChatEvalEvaluator extends BaseEvaluator {
  ...
  async evaluate(
    ...
    for (const probe of probes) {
      const score = await this.evaluateWithProbe(
        variant.prompt,
        response,
        probe,
        taskMeta
  ...
  private async evaluateWithProbe(
    ...
    const probePrompt = this.createProbePrompt(
      originalPrompt,
      response,
      probe
    )
    ...
    try {
      ...
      const llmResponse = await axios.post(`${llmRunnerUrl}/run`, {
      ...
      return await this.scoreProbeResponse(
...
RATING: [number 1-10]`
    ...
    try {
      const llmRunnerUrl = this.getLLMRunnerUrl()
      const judgment = await axios.post(`${llmRunnerUrl}/run`, {
