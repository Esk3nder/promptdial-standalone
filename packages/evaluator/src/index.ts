...
export class CalibratedEvaluatorEnsemble {
  ...
  async evaluate(
    ...
    const evaluationPromises = applicableEvaluators.map(evaluator =>
      this.runEvaluator(evaluator, variant, response, taskMeta, references)
        .catch(error => {
  ...
  async compareVariants(
    ...
    return results.sort((a, b) => 
      (b.evaluation.final_score || 0) - (a.evaluation.final_score || 0)
    )
  }
  ...
  private selectEvaluators(
    ...
    for (const evaluator of this.evaluators.values()) {
      ...
      if (variant.technique.includes('consistency') && 
  ...
  private shouldIncludeEvaluator(
    ...
    if (evaluator.getName() === EVALUATORS.CHAT_EVAL && 
        ['general_qa', 'creative_writing'].includes(taskMeta.task_type)) {
      return true
    }
    
    // Include role-debate for high complexity
    if (evaluator.getName() === EVALUATORS.ROLE_DEBATE && 
        taskMeta.complexity > 0.7) {
      return true
    }
  ...
  private applyCalibration(result: EvaluationResult): EvaluationResult {
    ...
    for (const [evaluator, score] of Object.entries(result.scores)) {
      calibratedScores[evaluator] = this.calibrationMonitor.calibrateScore(
        evaluator,
        score
      )
    }
...
export async function handleCompareRequest(
  ...
  try {
    ...
    const results = await getEvaluatorEnsemble().compareVariants(
      variants,
      task_meta,
      references
    )
