...
export class CalibrationMonitor {
  ...
  private calculateDrift(
    ...
    const oldDiffs = oldData.map(p => 
      p.evaluator_scores[evaluator] - p.human_score!
    )
    const newDiffs = newData.map(p => 
      p.evaluator_scores[evaluator] - p.human_score!
    )
  ...
  private async alertDrift(evaluator: string, stats: CalibrationStats): Promise<void> {
    ...
    logger.error(`ALERT: ${evaluator} drift ${stats.drift} exceeds threshold ${this.driftThreshold}`)
  }
}
