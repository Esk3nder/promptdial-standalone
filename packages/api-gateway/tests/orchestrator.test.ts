...
describe('RequestOrchestrator', () => {
  ...
  describe('optimize', () => {
    ...
    beforeEach(() => {
      ...
      mockAxios.post.mockImplementation((url: string) => {
        ...
        if (url.includes('/check-variant')) return Promise.resolve(mockResponses.safety_check_variant)
    ...
    it('should handle unsafe prompts', async () => {
      ...
      await expect(orchestrator.optimize(request, 'trace-123'))
        .rejects.toThrow('Contains harmful content')
    })
    ...
    it('should throw when no safe variants available', async () => {
      ...
      await expect(orchestrator.optimize(request, 'trace-123'))
        .rejects.toThrow('No safe variants available')
    })
  })
  ...
  describe('callService', () => {
    it('should make service call with correct parameters', async () => {
      ...
      const result = await (orchestrator as any).callService(
        'classifier',
        '/classify',
    ...
    it('should throw for unknown service', async () => {
      await expect(
        (orchestrator as any).callService('unknown', '/endpoint', {})
  ...
  describe('executeVariants', () => {
    ...
    it('should respect concurrency limit', async () => {
      const variants = Array(10).fill(null).map((_, i) => 
        createTestPromptVariant({ id: `v${i}` })
      )
    ...
    it('should throw when no LLM runner available', async () => {
      ...
      await expect(
        (orchestrator as any).executeVariants(variants, 'trace-123')
