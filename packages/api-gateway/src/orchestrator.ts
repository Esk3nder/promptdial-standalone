...
export class RequestOrchestrator {
  ...
  private async executeVariants(
    ...
    if (!llmRunnerUrl) {
      logger.error('No LLM Runner available - check API keys', { traceId })
      throw new Error('No LLM provider configured. Please set OPENAI_API_KEY, ANTHROPIC_API_KEY, or GOOGLE_AI_API_KEY in .env')
    }
  ...
  private async callLLMRunner(
    ...
    try {
      ...
      logger.error(`LLM Runner call failed for variant ${variant.id}`, {
        status: axiosError.response?.status,
        error: axiosError.message,
        latency,
        url
