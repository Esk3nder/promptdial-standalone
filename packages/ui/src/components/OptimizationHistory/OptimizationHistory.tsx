...
export function OptimizationHistory({
  ...
  const filteredHistory = history.filter((item) => {
    const matchesSearch =
      !searchQuery || item.request.prompt.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        {history.length > 0 && (
          <button className={styles.clearButton} onClick={onClear} aria-label="Clear all history">
            Clear All
          </button>
        )}
      </div>
                ...
                <div key={item.id} className={styles.historyItem}>
                  <button className={styles.itemButton} onClick={() => onSelect(item)}>
                    ...
                    <div className={styles.itemContent}>
                      <p className={styles.originalPrompt}>{item.request.prompt}</p>
                    ...
                    <div className={styles.itemMeta}>
                      <span className={styles.metaItem}>{item.request.targetModel}</span>
                      <span className={styles.metaItem}>{item.request.optimizationLevel}</span>
